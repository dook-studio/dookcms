...
partial class imgitem : BasePage
{

    #region 解析模板
    private Hashtable hs = new Hashtable();
    private DataRowView theimage;

    protected override void InitPageTemplate()
    {        
        this.Document.SetValue("sys", hs);
        this.Document.SetValue("the", theimage);
    ...
    protected override void LoadTemplateFile(string fileName)
    {
        ...
        DataRowView item = BLL.DataBaseHelper.instance.GetModelView("image", "*", "id=" + aid);
        if (item != null)
        {
            theimage = item;
            List<ImgItem> imgitems = Common.JsonHelper.FromJson<List<ImgItem>>(item["jsonstr"].ToString());
            ...
            if (model == null)
            {
                fileName = this.Server.MapPath("~/templets/" + template + "/imgitem.html");
            ...
            {
                Response.Write("你还没有设置该栏目(编号为:" + item["typeid"] + ",类型:图集列表)下的模板文件喔~^~!请在后台栏目管理->高级->内容模板中设置.设置示例:imgitem.html");
                Response.End();
                return;
            }
            fileName = this.Server.MapPath("~/templets/" + template + "/" + templatefile);
            base.LoadTemplateFile(fileName);
            this.Document.SetValue("imgitems", imgitems);
        }
        else
        {
            ...
            if (td.CurrentRenderingTag != null)
            {
                ...
                try
                {
                    if (get == "pre")
                    {
                        ...
                        DataRowView o = BLL.DataBaseHelper.instance.GetList("image", "id,title", 1, "id>" + aid, "id desc").Tables[0].DefaultView[0];
                        if (o != null)
                        {
                    ...
                    {
                        DataRowView o = BLL.DataBaseHelper.instance.GetList("image", "id,title", 1, "id<" + aid, "id").Tables[0].DefaultView[0];
                        if (o != null)
                        {
                            return string.Format("<a href=\"/item.ashx?aid={0}\">{1}</a>", o["id"].ToString(), o["title"].ToString());
                        }
