...
public partial class Crm_Article_Edit : BaseCrm
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (StringHelper.isNum(Request.QueryString["id"]))//如果是更新数据,则加载该记录.
            {
                ...
                DataRowView model = BLL.DataBaseHelper.instance.GetModelView("article", "*", "id=" + id);
                if (model != null)
                {
                    ...
                    txtBrief.Text = model["brief"].ToString();
                    txtAuthor.Text = model["author"].ToString();
                    txtSource.Text = model["source"].ToString();
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        hs.Add("tags", txtTitle.Text.Trim());
        hs.Add("brief", txtBrief.Text.Trim());
        hs.Add("author", txtAuthor.Text.Trim());
        hs.Add("source", txtSource.Text.Trim());
        ...
        if (!StringHelper.isNum(Request.QueryString["id"]))//插入新纪录
        {
            ...
            {
                hs.Add("addtime", DateTime.Now.ToString());
            }
            BLL.DataBaseHelper.instance.Insert(hs, "Article");
            txtTitle.Text = "";
            txtBrief.Text = "";
            txtBody.Value = "";
        ...
        {
            int id = int.Parse(Request.QueryString["id"]);            
            BLL.DataBaseHelper.instance.Update(hs, "Article", "[id]=" + id);
    ...
    {
        BLL.DataBaseHelper.instance.Delete("Article", "id=" + Request.QueryString["id"]);
